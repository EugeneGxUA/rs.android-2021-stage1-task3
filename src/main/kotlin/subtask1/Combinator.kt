...
class Combinator {
    ...
    let us search x (number of colors for each poster so that each poster has a unique combination of colors and the number of combinations is
    ...
    fun checkChooseFromArray(array: Array<Int>): Int? {
        if (array.size > 2 || array.size < 2 || array[0] < 0 || array[1] <= 0) {
            return null
        }
...
fun main() {
    val c = Combinator()
    c.checkChooseFromArray(arrayOf(7, 3))
}
